...
fprintf( stderr, "\n%s\n\n", "SYNOPSIS" );
fprintf( stderr, "%s\n", "netio [-l <target> [-1]] [-i <file>] [-o <file>] [-e <command>] [-c <command>] [-w <sec>] [-a] [-s] [-n] [-b <size[K|M]>] <target>" );
fprintf( stderr, "%s\n", "<target>          <local>|<net>" );
...
fprintf( stderr, "%s\n", "-e --exec         exec <command>, bind stdin and stdout with this program" );
...
fprintf( stderr, "\n%s\n", "$ netio unix:../mysocket" );
fprintf( stderr, "\n%s\n", "$ echo \"MBX7Z86PRQYUHH13NKMTNQJGZY6ECTE=\" | netio -w 3 -n 10.40.100.10:31337 >>answer" );
